...
import { BookSubNavigator } from '../TabsSubNavigation';
...
export const BookConversations = ({ book, userDataFull }: { book?: IBook, userDataFull: IAPI_User_Full }) => {
  ...
  const bookConversations = liveConversations?.filter(conversation => conversation?.book?.bookRef === book?._id);
  return (
    <BCContainer
      style={{ flex: 1, backgroundColor: palette.primary.linen }}
      pageTitle={book?.title}
      showBackButton={true}
      userImage={userDataFull?.images}
      userFullName={userDataFull?.name}>
      <BookSubNavigator routeName={ROUTE.BOOK_CONVERSATIONS} currentUserDataFull={userDataFull} book={book!}/>
